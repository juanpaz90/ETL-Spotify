...
SPOTIFY_EXTRACTOR_TAG="us-east1-docker.pkg.dev/$PROJECT_ID/$REPOSITORY_NAME/$IMAGE_NAME:latest"

# Build spotify-data-extractor image
echo " Build spotify-data-extractor container"
docker build -t "$SPOTIFY_EXTRACTOR_TAG" -f 'build-scripts/spotify_extractor.Dockerfile' .
if [ $? -ne 0 ]; then
